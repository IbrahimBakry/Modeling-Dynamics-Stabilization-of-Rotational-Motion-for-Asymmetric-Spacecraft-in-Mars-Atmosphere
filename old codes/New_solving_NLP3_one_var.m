...
fun = @(x) (576./(pi.*h.*(3.*((sqrt(pi).*x(1).*h)./sqrt(Smax-x(1).^2*pi)).^2+h.^2).*((x(1)./((sqrt(pi).*x(1).*h)./sqrt(Smax-x(1).^2*pi))).^2)./3)).*(pi/2).*(x(1)./((sqrt(pi).*x(1).*h)./sqrt(Smax-x(1).^2*pi))).^4.*(h.^5./5 + (2.*((sqrt(pi).*x(1).*h)./sqrt(Smax-x(1).^2*pi)).^2.*h.^3)./3 + ((sqrt(pi).*x(1).*h)./sqrt(Smax-x(1).^2*pi)).^4.*h);
