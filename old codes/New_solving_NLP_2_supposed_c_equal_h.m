...
fun = @(x) (576./(pi.*h.*(3.*h.^2+h.^2).*((x(1)./h).^2)./3)).*(pi/2).*(x(1)./h).^4.*(h.^5./5 + (2.*h.^2.*h.^3)./3 + h.^4.*h);

% Nonlinear Constraints (cl <= nlcon(x) <= cu)
nlcon = @(x) [ (1/3).*pi.*h.*(x(1)./h).^2.*(3*h.^2+h.^2); % Vol
               pi*(x(1)./h).^2.*(h.^2+h.^2)]; % Are
...
Volopt(k) = (1/3).*pi.*h.*(A_opt(k)./C_opt(k)).^2.*(3*C_opt(k).^2+h.^2);
