%% this program find the minimum values of Iz1 for range h a c
clc; clear; close all;

Vmin = 1.820219934;
Smax = 4.523893422;
hstar= 0.5;%0.67;
hend = 3*Vmin/Smax;
hstep= 0.08;

% Parameters Intervals
a = [0.1:0.005:5];
c = [0.1:0.005:5];

k = 0; 
for h = hstar:hstep:hend
    k = k + 1; ii = 0; q = 0;
    for i = 1:length(a)
        ii = ii + 1; jj = 0;
        for j = 1:length(c)
            jj = jj + 1;
            Vol = (1/3).*pi.*h.*(a(i)./c(j)).^2.*(3*c(j).^2+h.^2);
            Are = pi*(a(i)./c(j)).^2.*(h.^2+c(j).^2);

            % Checking the condition and selecting a & c
            if (Vol >= Vmin) && (Are <= Smax) && (Are > 0)
                q = q + 1;
                A(q) = a(i);
                C(q) = c(j);
                IZ(q) = (576./(pi.*h.*(3.*C(q).^2+h.^2).*((A(q)./C(q)).^2)./3)).*(pi/2).*(A(q)./C(q)).^4.*(h.^5./5 + (2.*C(q).^2.*h.^3)./3 + C(q).^4.*h);
                IZAC(q,:) = [A(q), C(q), IZ(q)];
            end
        end
    end
    if q <= 1
        disp('There are no values of a & c that satisfy the Conditions')
    else
    % Getting the minimum value
    IZ_min(k) = min(IZAC(:,3));
    IZ_max(k) = max(IZAC(:,3));
    
    val = find(IZ==IZ_min(k));
    A_opt(k) = A(val);
    C_opt(k) = C(val);
    end
    
    %% Error here, h hould be a vector not a scaler

Vol = (1/3).*pi.*h.*(A_opt./C_opt).^2.*(3*C_opt.^2+h.^2);
Are = pi*(A_opt./C_opt).^2.*(h.^2+C_opt.^2);
rho = 576./(pi.*h.*(3.*C_opt.^2+h.^2).*((A_opt./C_opt).^2)./3);
Ix = rho.*pi.*(A_opt/C_opt).^2.*(h.^5./5 + C_opt.^2.*h.^3./3) + rho.*(pi./4).*(A_opt./C_opt).^4.*(h.^5./5 + 2.*C_opt.^2.*h.^3./3 + C_opt.^4.*h);

end


% Omegaz Law without aerodynamics
omega0 = 1;
Iz10 = IZ_min(1);
Iz2 = 270;
Iz1 = IZ_min;
omega = (Iz10 + Iz2).*omega0./(Iz1 + Iz2);

figure(10); plot([hstar:hstep:hend],omega,'k','linewidth',4);ylabel('\omega_z'); xlabel('h');
ax = gca; ax.GridLineStyle = ':'; ax.GridAlpha = 0.7; ax.FontSize = 16; ax.FontWeight= 'bold'; ax.LineWidth = 0.8; 
xticks([0.67 0.72 0.77 0.82 0.87 0.92 0.97 1.02 1.08 1.14 1.2])
ax.XAxis.LineWidth = 2; ax.YAxis.LineWidth = 2;
% yticks([140 155 170 185 200 215 230 245 262])
% ax.YLim = [140 262]; 
ax.XLim = [0.67 1.2];

figure(11); plot(IZ_min,omega*57.3,'k','linewidth',4);ylabel('\omega_z'); xlabel('IZmin');
ax = gca; ax.GridLineStyle = ':'; ax.GridAlpha = 0.7; ax.FontSize = 16; ax.FontWeight= 'bold'; ax.LineWidth = 0.8; 
% xticks([0.67 0.72 0.77 0.82 0.87 0.92 0.97 1.02 1.08 1.14 1.2])
ax.XAxis.LineWidth = 2; ax.YAxis.LineWidth = 2;
% yticks([140 155 170 185 200 215 230 245 262])
% ax.YLim = [140 262]; 
% ax.XLim = [0.67 1.2];

figure(1); plot([hstar:hstep:hend],IZ_min,'linewidth',4);ylabel('Iz'); xlabel('h'); %axis([0.7 1.19 140 250])
ax = gca; ax.GridLineStyle = ':'; ax.GridAlpha = 0.7; ax.FontSize = 16; ax.FontWeight= 'bold'; ax.LineWidth = 0.8; 
xticks([0.67 0.72 0.77 0.82 0.87 0.92 0.97 1.02 1.08 1.14 1.2])
ax.XAxis.LineWidth = 2; ax.YAxis.LineWidth = 2;
yticks([140 155 170 185 200 215 230 245 262])
ax.YLim = [140 262]; ax.XLim = [0.67 1.2];

figure(2); plot([hstar:hstep:hend],A_opt,'linewidth',4);ylabel('a'); xlabel('h'); %axis([hstar 1.19 -Inf Inf])
ax = gca; ax.GridLineStyle = ':'; ax.GridAlpha = 0.7; ax.FontSize = 16; ax.FontWeight= 'bold'; ax.LineWidth = 0.8; 
xticks([0.67 0.72 0.77 0.82 0.87 0.92 0.97 1.02 1.08 1.14 1.2])
ax.XAxis.LineWidth = 2; ax.YAxis.LineWidth = 2;
yticks([0.6 0.62 0.64 0.66 0.68 0.7 0.72 0.74 0.76 0.78 0.8 0.82])
ax.YLim = [0.6 0.82]; ax.XLim = [0.67 1.2];

figure(3); plot([hstar:hstep:hend],C_opt,'linewidth',4);ylabel('c'); xlabel('h'); %axis([0.68 1.2 -Inf Inf])
ax = gca; ax.GridLineStyle = ':'; ax.GridAlpha = 0.7; ax.FontSize = 16; ax.FontWeight= 'bold'; ax.LineWidth = 0.8; 
xticks([0.67 0.72 0.77 0.82 0.87 0.92 0.97 1.02 1.08 1.14 1.2])
ax.XAxis.LineWidth = 2; ax.YAxis.LineWidth = 2;
yticks([0.665 0.7 0.75 0.8 0.85 0.9 0.95 1 1.05 1.1 1.15 1.2])
ax.YLim = [0.665 1.2]; ax.XLim = [0.67 1.2];

figure(4); plot([hstar:hstep:hend],Vol,[0.67:0.01:1.2],1.8202*ones(1,length([0.67:0.01:1.2])),'linewidth',4); xlabel('h'); ylabel('Volume'); %axis([hstar 1.19 -Inf Inf])
ax = gca; ax.GridLineStyle = ':'; ax.GridAlpha = 0.7; ax.FontSize = 16; ax.FontWeight= 'bold'; ax.LineWidth = 0.8; 
xticks([0.67 0.72 0.77 0.82 0.87 0.92 0.97 1.02 1.08 1.14 1.2])
ax.XAxis.LineWidth = 2; ax.YAxis.LineWidth = 2;
yticks([1.5 2.00 2.50 3.00 3.50 4.00 4.5 5])
ax.YLim = [1.5 5]; ax.XLim = [0.67 1.2];

figure(5); plot([hstar:hstep:hend],Are,[0.67:0.01:1.2],4.52389*ones(1,length([0.67:0.01:1.2])),[0.67:0.01:1.2],zeros(1,length([0.67:0.01:1.2])),'linewidth',4); xlabel('h'); ylabel('Area'); %axis([hstar 1.19 -Inf Inf])
ax = gca; ax.GridLineStyle = ':'; ax.GridAlpha = 0.7; ax.FontSize = 16; ax.FontWeight= 'bold'; ax.LineWidth = 0.8; 
xticks([0.67 0.72 0.77 0.82 0.87 0.92 0.97 1.02 1.08 1.14 1.2])
ax.XAxis.LineWidth = 2; ax.YAxis.LineWidth = 2;
yticks([0 0.5 1 1.5 2 2.5 3 3.5 4 4.52 5])
ax.YLim = [0 5]; ax.XLim = [0.67 1.2];

figure(7); plot([hstar:hstep:hend],Ix,'linewidth',4);ylabel('Ix'); xlabel('h'); axis([hstar 1.19 -Inf Inf])
ax = gca; ax.GridLineStyle = ':'; ax.GridAlpha = 0.7; ax.FontSize = 16; ax.FontWeight= 'bold'; ax.LineWidth = 0.8; 
ax.YLim = [350 750]; ax.XLim = [0.67 1.2];
xticks([0.67 0.72 0.77 0.82 0.87 0.92 0.97 1.02 1.08 1.14 1.2])
ax.XAxis.LineWidth = 2; ax.YAxis.LineWidth = 2;

figure(8); % a and c togather
yyaxis left; plot([hstar:hstep:hend],A_opt,'k','linewidth',4); %ylabel('a')
yticks([0.6 0.62 0.64 0.66 0.68 0.7 0.72 0.74 0.76 0.78 0.8 0.82])
ax = gca; ax.GridLineStyle = ':'; ax.GridAlpha = 0.7; ax.FontSize = 16; ax.FontWeight= 'bold'; ax.LineWidth = 0.8; 
ax.YLim = [0.6 0.82]; 
yyaxis right; plot([hstar:hstep:hend],C_opt,':dk','linewidth',4);%ylabel('c')
yticks([0.665 0.7 0.75 0.8 0.85 0.9 0.95 1 1.05 1.1 1.15 1.2])
ax.YLim = [0.665 1.2]; 
ax.XAxis.LineWidth = 3;
ax.YAxis(1).LineWidth = 3;
ax.YAxis(2).LineWidth = 3;
grid on; xlabel('h');
xticks([0.67 0.72 0.77 0.82 0.87 0.92 0.97 1.02 1.08 1.14 1.2])
legend('a values.....','c values.....')
ax.XLim = [0.67 1.2];

figure(9); % Iz, Ix togather
yyaxis left; plot([hstar:hstep:hend],IZ_min,'k','linewidth',4); %ylabel('a')
yticks([140 155 170 185 200 215 230 245 262])
ax = gca; ax.GridLineStyle = ':'; ax.GridAlpha = 0.7; ax.FontSize = 16; ax.FontWeight= 'bold'; ax.LineWidth = 0.8; 
ax.YLim = [140 262];
yyaxis right; plot([hstar:hstep:hend],Ix,'-.k','linewidth',4);%ylabel('c')
yticks([350 400 450 500 550 600 650 700 750]); ax.YLim = [0.665 1.2];
ax.XAxis.LineWidth = 3; ax.YAxis(1).LineWidth = 3; ax.YAxis(2).LineWidth = 3;
xlabel('h'); ax.YLim = [350 750]; ax.XLim = [0.67 1.2];
xticks([0.67 0.72 0.77 0.82 0.87 0.92 0.97 1.02 1.08 1.14 1.2])
legend('Iz values.....','Ix values.....')

